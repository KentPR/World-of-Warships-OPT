...
void addelem_B_list(int deck, int type, int num)
{
	...
	for (int n = 0; n < deck; n++) {
		b_temporary->b_ship[n][0] = B_matryx[n][0];
		b_temporary->b_ship[n][1] = B_matryx[n][1];
	}
...
void create_mass_of_targets() {// test
	int count = 0;
		...
		for (int l = -1; l < 2; l++) {
			for (int m = -1; m < 2; m++) {
				...
				if (l != 0 || m != 0) {// исключение самой точки
					...
					if ((!huina) && (coord1 + l >= 0 && coord1 + l < 10) && (coord2 + m >= 0 && coord2 + m < 10)) {//выход за границы пол€
...
bool finishing() {
	//выбирает вариацию смещени€ выстрела относительно 1го попадани€
	...
	if (destroed_deck_B == deck_of_ship) {
		...
		while (next_strike < 4) {// 6, 3, 12, 9  часов соответственно
			...
			if (strike_is_correct_B(coord_strike1, coord_strike2)) {
...
*/
	...
	while (!strike_is_correct_B(X_coord, Y_coord)) {
		X_coord = rand() % 10;
		Y_coord = rand() % 10;
...
bool B_Check(int deck)// true
{
	...
	for (int b = 0; b < deck; b++) {
		if ((B_matryx[b][0] > 9) || (B_matryx[b][0] < 0) || (B_matryx[b][1] < 0) || (B_matryx[b][1] > 9))
		{
			//printf("err1");
			//_getch();
			return true;
	...
	for (int a = 0; a < B_last_point; a++) {
		for (int b = 0; b < deck; b++) {
			if (B_ban[a][0] == B_matryx[b][0] && B_ban[a][1] == B_matryx[b][1]) {
				//printf("err2");
				//_getch();
				return true;
			}
...
bool Player_Atack() {
	...
	int X_coord = -1;
	int Y_coord = -1;
	while (!strike_is_correct_P(X_coord, Y_coord)) {
